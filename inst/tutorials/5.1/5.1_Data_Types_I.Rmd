...
You create them using a function you've already seen, `c()`. The c is short for *combine* or *concatenate*.
...
There's one other important way to create vectors - the colon `:` notation you may remember from `select()`. In `select()` it allowed you to select a bunch of adjacent columns. It works much the same way when creating vectors by stringing together a bunch of consecutive numbers.
...
**Strings are always enclosed in quotation marks!** This is how we differentiate them from numbers and object names. For example, `Charis` refers to an object named `Charis` - this could be a vector of numbers, or a data frame, or anything! But `"Ralph"` refers to a string, a bit of text - that is, the literal letters r, a, l, p, and h next to each other. 
...
char_vec_2 <- c("Happy", "Birthday", "To", "You", "!", "You are 19 years old.")
char_vec_2
...
> Fun fact: The term "double" comes from the amount of computer memory needed to hold a decimal number with a certain amount of precision; one way, a "float", takes 32 bits, but the more precise "double" uses 64 bits. 

By default R will store numbers as **doubles**. To specify you want an integer you have to use `L` afterwards. I have no idea why; please don't ask.
...
Let's start by using numbers to represent positions in the vector:

```{r vec_sub1, exercise = TRUE}
# Create a vector
bday <- c("Happy", "Birthday", "To", "You", "!", "You are 19 years old.")
...
This may seem arbitrary now, but I assure you it's actually quite useful (**hint hint** like for an upcoming homework)!
...
**End of sidenote.** Let's look at a slightly more complex example. 
...
Is your brain melting yet? I hate this. Let's move on.
