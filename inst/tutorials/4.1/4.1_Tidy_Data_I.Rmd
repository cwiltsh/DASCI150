...
author: 
- "Zachary Binney, PhD"
- "Modified by Charis Wiltshire, MPH"
date: "February 2026"
...
I warned you, and it's time. It's time to learn techniques for helping with what **data scientists** report they spend most of their day doing: **data cleaning**. It's most of what they do - we have to teach you, no way around it.

For what it's worth, data cleaning *can* actually be fun. Think of it like an interesting challenge or puzzle. Not to mention the dopamine rush when you finally get your code to run and produce a really clean data set for others to use.
...
These data frames were either included in R natively, part of packages we loaded, or found independently and cleaned by Dr. Binney before being provided to you.

```{r img0, echo = FALSE, out.width = "100%", fig.align = "center", fig.cap = "A nice and tidy dataset."}
knitr::include_graphics("images/tidy_data.png")
```

But in the real world data doesn't come to you like the image above. It comes as a mess. Even from very professional sources like the World Bank. It's more like the image below.

```{r img1, echo = FALSE, out.width = "100%", fig.align = "center", fig.cap = "[messy real-world data](https://databank.worldbank.org/source/world-development-indicators/preview/on)."}
knitr::include_graphics("images/messy.png")
```

That data from the World Bank may not *look* that hard to work with or like it requires cleaning or tidying, but think about it for a moment. What does each row represent? Is each column a different variable, like we're used to seeing? If not, what is it? Where are the variables? Could we easily `select()` certain ones we want, or manipulate them with the techniques we learned?
...
Like any good epidemiologist would say, it depends.

In short, NO! The key message here is that while tidy data is often an excellent default, you shouldn't get fanatical about it. Be practical and process-oriented. It's fine to use a data format that works well for what you need to do.
...
We'll learn in the next Tutorial how to switch between long and wide data seamlessly. 
...
Bonus points if you can figure out how to get rid of that nasty hyphen at the start of our month-day column values. This is left as an exercise for the reader.
