...
quiz(
  question("What are the first two **arguments** `aes()` expects?",
      answer("y =, then x ="),
      answer("x =, then y =", correct = TRUE, message = "That's right! When you provide values to `aes()`, R will assign the first one to `x =`, and the second one to `y =`."),
      answer("x =, then fill ="),
      answer("I don't know"),
      type = "learnr_radio",
      allow_retry = TRUE)
...
#Try this as-is, then wrap the entire thing in parentheses (that is, "(" before abq_flights and ")" at the end of the line)
abq_flights <- filter(flights, dest == "ABQ")
```
...
bday_flights <- filter(flights, month == 8 & day == 4)
```

Here I wanted only flights with *both* `month == 8` *and* `day == 4`. To specify this I used the **boolean operator** `&` (an ampersand, meaning "AND"). The major operators are:
...
One last example: What if instead of only looking for flights to Atlanta or Albuquerque not operated by United, I wanted to *exclude* those flights. In other words, I want flights NOT (to Atlanta or Albuquerque AND NOT operated by United). You can negate an entire sequence of other criteria by using `!` and parentheses:
...
```
We have some tiny carriers with weird departure patterns are completely skewing the graph. It turns out they're "HA" and "OO". Let's get rid of them.
...
While `filter()` chooses a subset of rows, `arrange()` re-arranges (or sorts) the rows based on the columns you specify. By default it sorts them in **ascending** order (lowest-to-highest). For example, let's sort flights from earliest to latest scheduled departure time:
